...
export default function Module1Page({ onBack, onLogout, resumeVisit }) {
  ...
  useEffect(() => {
    ...
    recordModuleVisit(firebaseUser?.uid, {
      route: "module-1",
      activity: experienceStep === "components" ? `${selectedPlatform.toUpperCase()} · ${current.title || current.name || current.key}` :
        ({ welcome: "Introduction", platform: "Platform selection", tutorial: "Tutorial" }[experienceStep] || "Introduction"),
      snapshot: { selectedPlatform, moduleIndex: safeModuleIndex, experienceStep, showIntro, tutorialPage },
    });
  }, [profile, firebaseUser?.uid, selectedPlatform, safeModuleIndex, experienceStep, showIntro, tutorialPage, current]);
  ...
  useEffect(() => {
    ...
    if (resume && ["", "amd", "intel"].includes(resume.selectedPlatform) &&
        ["welcome", "platform", "tutorial", "components"].includes(resume.experienceStep)) {
      setSelectedPlatform(resume.selectedPlatform);
      setModuleIndex(Math.max(0, Math.min(Number(resume.moduleIndex) || 0, modules.length - 1)));
      setExperienceStep(resume.experienceStep);
      setShowIntro(resume.showIntro !== false);
      setTutorialPage(Math.max(0, Math.min(Number(resume.tutorialPage) || 0, 2)));
      setLocalCompletedParts(getModule1CompletedParts(resume.selectedPlatform));
      return;
    }
  ...
  }, [profile, modules.length, resumeVisit]);
