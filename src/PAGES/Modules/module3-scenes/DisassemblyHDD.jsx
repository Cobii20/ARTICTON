...
const PSU_POSITION = new THREE.Vector3(4.27, -15.66, 6.22);
const PSU_ROTATION = new THREE.Euler(0, Math.PI, 0);

const HDD_INSTALLED_POS = new THREE.Vector3(4.16, -14.32, -0.49);
const HDD_INSTALLED_ROT = new THREE.Euler(0, 0, 0);

const HDD_FLOOR_POS = new THREE.Vector3(9.7, -13.75, 0.5);
const HDD_FLOOR_ROT = new THREE.Euler(Math.PI / 2, 0, 0);
  ...
  return (
    <Html position={[-10, 8.5, 0]} style={{ pointerEvents: "auto" }}>
      <div style={{ minWidth: 240, padding: 12, borderRadius: 14, background: "rgba(10,14,22,.80)", border: "1px solid rgba(0,255,180,.18)", color: "#eef4ff", fontFamily: "monospace" }}>
        ...
        <div style={{ marginBottom: 8 }}>DisassemblyHDD.jsx</div>
        <div style={{ marginBottom: 10 }}>Remove the HDD from the bay and place it neatly on the floor area.</div>
        <button onClick={onToggle} style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,.16)", background: detached ? "#00ffb4" : "rgba(255,255,255,.06)", color: detached ? "#081018" : "#eef4ff", fontWeight: "bold", cursor: "pointer" }}>
          {detached ? "Reinstall HDD" : "Detach HDD"}
        </button>
      </div>
    </Html>
  );
...
function Scene() {
  ...
  useEffect(() => {
    ...
    hddRef.current.position.copy(HDD_INSTALLED_POS);
    hddRef.current.rotation.copy(HDD_INSTALLED_ROT);
  }, []);

  useFrame(() => {
    animatePart(
      hddRef,
      detached ? HDD_FLOOR_POS : HDD_INSTALLED_POS,
      detached ? HDD_FLOOR_ROT : HDD_INSTALLED_ROT
    );
  });
  ...
  return (
    <>
      ...
      <group position={PSU_POSITION} rotation={PSU_ROTATION.toArray()}><primitive object={psuClone} /></group>

      <group ref={hddRef}>
        <primitive object={hddClone} />
        <Html position={[0, 1.2, 0]} center>
          <div style={{ padding: "5px 8px", borderRadius: 10, background: "rgba(0,0,0,.55)", color: "#fff", fontSize: 11 }}>
            {detached ? "HDD on floor" : "HDD installed"}
          </div>
        </Html>
      </group>
...
export default function DisassemblyHDD() {
  return (
