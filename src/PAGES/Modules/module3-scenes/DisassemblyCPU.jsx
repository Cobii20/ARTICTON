...
const CPU_INSTALLED_POS = new THREE.Vector3(-1.25, -4.95, -2.66);
const CPU_INSTALLED_ROT = new THREE.Euler(Math.PI / 2, 0, -Math.PI / 2);

const CPU_FLOOR_POS = new THREE.Vector3(5.5, -13.85, -2.2);
const CPU_FLOOR_ROT = new THREE.Euler(Math.PI / 2, 0.2, 0);
  ...
  return (
    <Html position={[-10, 8.5, 0]} style={{ pointerEvents: "auto" }}>
      <div style={{ minWidth: 240, padding: 12, borderRadius: 14, background: "rgba(10,14,22,.80)", border: "1px solid rgba(0,255,180,.18)", color: "#eef4ff", fontFamily: "monospace" }}>
        ...
        <div style={{ marginBottom: 8 }}>DisassemblyCPU.jsx</div>
        <div style={{ marginBottom: 10 }}>Remove the CPU and place it on the floor layout in front.</div>
        <button onClick={onToggle} style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,.16)", background: detached ? "#00ffb4" : "rgba(255,255,255,.06)", color: detached ? "#081018" : "#eef4ff", fontWeight: "bold", cursor: "pointer" }}>
          {detached ? "Reinstall CPU" : "Detach CPU"}
        </button>
      </div>
    </Html>
  );
...
function Scene() {
  ...
  useEffect(() => {
    ...
    cpuRef.current.position.copy(CPU_INSTALLED_POS);
    cpuRef.current.rotation.copy(CPU_INSTALLED_ROT);
  }, []);

  useFrame(() => {
    animatePart(
      cpuRef,
      detached ? CPU_FLOOR_POS : CPU_INSTALLED_POS,
      detached ? CPU_FLOOR_ROT : CPU_INSTALLED_ROT
    );
  });
      ...
      <group ref={cpuRef}>
        ...
        <Html position={[0, 1.2, 0]} center>
          <div style={{ padding: "5px 8px", borderRadius: 10, background: "rgba(0,0,0,.55)", color: "#fff", fontSize: 11 }}>
            {detached ? "CPU on floor" : "CPU installed"}
          </div>
        </Html>
      </group>
...
export default function DisassemblyCPU() {
  return (
