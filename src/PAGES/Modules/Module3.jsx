...
function Module3PlatformChoice({
  user,
  onBack,
  onLogout,
  onSelectPlatform,
  ...
  const platforms = [
    {
      ...
      name: "AMD",
      detail:
        "AM4 and AM5 platforms with socket, motherboard, and memory compatibility paths.",
    },
    {
      id: "intel",
      name: "Intel",
      detail:
        "Intel Core platforms with matching sockets, chipsets, and supported memory generations.",
    },
  ];
      ...
      <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden p-4 md:p-8">
        ...
        <div className="relative z-10 flex flex-col overflow-hidden rounded-[30px] border border-[#1a2438] bg-[linear-gradient(135deg,#0a0e17,#0d1220,#101a2d)] shadow-[0_70px_180px_rgba(0,0,0,0.70)]"
          ...
          <div className="relative z-10 flex flex-1 items-center justify-center px-6 py-7 md:px-8 md:py-8">
            <PlatformChoicePanel
              title="Choose your assembly platform"
              subtitle="Select the processor platform you want to use for this assembly path."
              platforms={platforms}
...
export default function Module3Disassembly({
  ...
  const activityLastRecordedAtRef = useRef(Date.now());

  const recordModuleActivity = async () => {
    if (!firebaseUser || !profile) return;

    const now = Date.now();
    const minutes = Math.floor((now - activityLastRecordedAtRef.current) / 60000);

    if (minutes <= 0) return;

    activityLastRecordedAtRef.current = now;

    try {
      await addUserActivity(firebaseUser.uid, profile, minutes);
    } catch (err) {
