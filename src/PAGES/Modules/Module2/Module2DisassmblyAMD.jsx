...
export default function Module2DisassemblyAMD({ onFinish, onBack, onLogout, onSwitchPlatform }) {
  const [checkpoint] = useState(() => getPracticeCheckpoint(auth.currentUser?.uid, "module-2-amd", steps, REMOVAL_SEQUENCE));
  ...
  useEffect(() => {
    recordModuleVisit(firebaseUser?.uid, {
      route: "module-2-amd",
      activity: "AMD" + " · " + (showIntro ? "Disassembly introduction" : currentStep.name),
