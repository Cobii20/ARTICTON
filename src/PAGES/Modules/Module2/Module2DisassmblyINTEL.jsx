...
function PartModel({
  ...
  // A checkpoint restores logical completion; restore its visual pose too.
  // Reset and final-round transitions remount the viewer with a fresh part list.
  useLayoutEffect(() => {
    if (!isCompleted || !restoreDisassemblyPlacement(
      groupRef.current, rotationRef.current, targetPosition, detachedQuaternion
    )) return;
  ...
  }, [isCompleted, targetPosition, detachedQuaternion, setPhaseSafely]);
...
function ModelViewer({
  ...
  const syncFullscreenState = useCallback(() => {
    ...
    const fullscreenTarget =
      viewerRef.current?.closest("[data-articton-workspace-fullscreen]") || viewerRef.current;
...
export default function Module2DisassemblyINTEL({ onFinish, onBack, onLogout, onSwitchPlatform }) {
  const [checkpoint] = useState(() => getPracticeCheckpoint(auth.currentUser?.uid, "module-2-intel", steps, REMOVAL_SEQUENCE));
  ...
  useEffect(() => {
    recordModuleVisit(firebaseUser?.uid, {
      route: "module-2-intel",
      activity: "INTEL" + " · " + (showIntro ? "Disassembly introduction" : currentStep.name),
  ...
  const saveFinalCompletion = useCallback(async () => {
    ...
    try {
      await completeGuidedModule("module2INTEL");
      const achievement = resolveAchievement("module2", { platform: "INTEL" });
                ...
                {isFinalRound && finalRoundCompletedParts.length === REMOVAL_SEQUENCE.length ? (
                  <div className="absolute inset-0 z-[760] flex items-center justify-center bg-[#050912]/78 p-5 backdrop-blur-md" role="status">
                    <div className="articton-nu-completion-card w-full max-w-xl rounded-[28px] border border-[#00ffb4]/35 bg-[#0b1220]/96 p-7 text-center shadow-[0_35px_100px_rgba(0,0,0,0.65)]">
                      ...
                      <button type="button" onClick={() => setShowCertificate(true)} className="mt-6 rounded-2xl bg-[#00ffb4] px-6 py-3 text-sm font-black text-[#07111d]">View Certificate</button>
                    </div>
                  </div>
                ) : null}
                <Sidebar
