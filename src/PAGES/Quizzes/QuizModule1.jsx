...
import { Bounds, Environment, OrbitControls, useGLTF, Html } from "@react-three/drei";
...
/* ───────────────────────────────────────────────────────────── */
/* PREPARED MODULE 1 QUESTIONS */
/* ───────────────────────────────────────────────────────────── */

const MODULE1_QUESTIONS = [
  {
    type: "choice",
    q: "Which component is commonly called the brain of the computer?",
    options: ["CPU", "RAM", "HDD", "PSU"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What is the main purpose of the motherboard?",
    options: [
      "It connects and allows the main components to communicate",
      "It only stores user files",
      "It supplies electricity by itself",
      "It cools the room around the computer",
    ],
    answerIndex: 0,
  },
  {
  ...
  {
    type: "choice",
    q: "Which component stores files and data for long-term use?",
    options: ["HDD", "RAM", "CPU", "Power button"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What does PSU stand for?",
    options: ["Power Supply Unit", "Personal Storage Unit", "Processor Support Utility", "Primary System Unit"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What is the main purpose of the PC case?",
    options: [
      "To hold and protect the internal components",
  ...
  {
    type: "choice",
    q: "Which part supplies electrical power to the computer components?",
    options: ["PSU", "CPU", "RAM", "HDD"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Where is the CPU installed?",
    options: ["On the motherboard CPU socket", "Inside the HDD", "Inside the PSU", "On the case side panel"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Where are RAM modules usually installed?",
    options: ["DIMM slots on the motherboard", "Drive bay", "PSU fan", "Case feet"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which component usually has many ports, slots, and connectors for other parts?",
    options: ["Motherboard", "RAM", "HDD", "Case screw"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What is a hard disk drive mainly used for?",
    options: ["Saving files, apps, and operating system data", "Powering the CPU", "Cooling the RAM", "Connecting the monitor cable only"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which part should you handle carefully because its contacts or pins can be damaged?",
    options: ["CPU", "Case panel", "Power cord only", "Computer desk"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What does RAM lose when the computer is turned off?",
    options: ["The temporary data it was holding", "The physical RAM slot", "The power supply", "The PC case"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which component should be connected to power cables from the PSU?",
    options: ["Components that need electricity, such as motherboard and drives", "Only the PC table", "Only the monitor stand", "Only the keyboard letters"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which part gives the computer its physical frame?",
    options: ["Case", "CPU", "RAM", "HDD platter"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which of these is a correct pair?",
    options: ["CPU — processes instructions", "RAM — supplies power", "PSU — stores files", "HDD — holds the motherboard"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which of these is a correct pair?",
    options: ["RAM — temporary memory", "Case — processor", "CPU — storage drive", "HDD — power converter"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Why is identifying each PC part important before assembly or disassembly?",
    options: [
      "It helps place and handle each component correctly",
      "It makes the computer heavier",
      "It removes the need for safety",
      "It changes the color of all parts",
    ],
    answerIndex: 0,
  },
];
...
/* ───────────────────────────────────────────────────────────── */
/* 3D / PIN-BASED QUESTIONS */
/* ───────────────────────────────────────────────────────────── */

const MODEL_QUESTIONS = [
  {
    type: "model",
    q: "Pin A is pointing to this 3D part. What component is it?",
  ...
  {
    type: "model",
    q: "Pin B identifies this 3D part from Module 1. What is it?",
    modelSrc: MODEL_URLS.motherboard,
    pinLabel: "B",
    options: ["Motherboard", "PC Case", "RAM", "HDD"],
    answerIndex: 0,
  },
  {
    type: "model",
    q: "Pin C is placed on this component. What part is shown?",
    modelSrc: MODEL_URLS.ram,
    pinLabel: "C",
    options: ["PSU", "RAM", "CPU", "Case"],
    answerIndex: 1,
  },
  {
    type: "model",
    q: "Pin D marks this storage component. What is it?",
    modelSrc: MODEL_URLS.hdd,
    pinLabel: "D",
    options: ["HDD", "RAM", "Motherboard", "CPU"],
    answerIndex: 0,
  },
  {
    type: "model",
    q: "Pin E points to the component that supplies power. What is it?",
    modelSrc: MODEL_URLS.psu,
    pinLabel: "E",
    options: ["CPU", "PSU", "RAM", "HDD"],
    answerIndex: 1,
  },
  {
    type: "model",
    q: "Pin F identifies the outer frame of the computer. What component is shown?",
    modelSrc: MODEL_URLS.case,
    pinLabel: "F",
    options: ["PC Case", "CPU", "RAM", "Motherboard"],
    answerIndex: 0,
  },
  {
    type: "model",
    q: "Look at the pinned 3D model. Which function matches this part?",
    modelSrc: MODEL_URLS.ram,
    pinLabel: "G",
    options: [
      "Temporary memory while programs are running",
      "Main power conversion",
      "Permanent mechanical storage only",
...
function buildQuizQuestions() {
  ...
  MODULE1_QUESTIONS.forEach((question, index) => {
    mixed.push(question);
...
function PinBadge({ label = "A" }) {
  return (
    <Html position={[0.65, 0.55, 0]} center style={{ pointerEvents: "none" }}>
      <div className="flex items-center gap-2 rounded-full border border-[#00ffb4]/45 bg-[#061E29]/90 px-3 py-2 text-xs font-black text-[#b7fff0] shadow-[0_12px_35px_rgba(0,255,180,0.18)] backdrop-blur-md">
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#00ffb4] text-[#061E29]">{label}</span>
        <span>PIN</span>
      </div>
    </Html>
  );
...
function ModelQuestionViewer({ modelSrc, pinLabel }) {
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden rounded-[24px] border border-[#5F9598]/25 bg-[#092532]/80 shadow-[0_24px_80px_rgba(0,0,0,0.32)] backdrop-blur-xl">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/10 px-5 py-3">
        <div>
          <div className="text-[13px] font-black text-white">3D Pin Viewer</div>
          <div className="mt-0.5 text-[11px] text-white/45">Use the pin and model shape to identify the component.</div>
        </div>
...
function PartsReviewVisual() {
  const parts = ["CPU", "MB", "RAM", "HDD", "PSU", "Case"];
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-center overflow-hidden rounded-[22px] border border-white/10 bg-[#092532]/70 p-5 shadow-[inset_0_0_80px_rgba(0,255,180,0.04)]">
      <div className="text-[12px] font-bold uppercase tracking-[0.24em] text-[#b7fff0]/70">Parts Review</div>
      <div className="mt-5 grid grid-cols-6 gap-2">
        {parts.map((part, index) => (
          <div key={part} className="relative">
            ...
            <div className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full border border-[#061E29] bg-[#00ffb4] text-[10px] font-black text-[#061E29] shadow-[0_0_12px_rgba(0,255,180,0.55)]">
              {index + 1}
            </div>
          </div>
        ))}
      ...
      <div className="mt-5 text-sm leading-6 text-white/50">
        These are the main parts introduced in Module 1. Use their name, shape, and function to answer.
      </div>
    </div>
  );
}
...
export default function QuizModule1({ onBack, onQuizComplete }) {
  ...
  const moduleCompleted =
    ...
    Object.values(moduleProgress?.completedParts || {}).filter(Boolean).length >= 6;
      ...
      <FullscreenShell>
        <CenteredPanel>
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-white/60">
            {accessError ? accessError : "Finish Module 1 first before taking this quiz."}
          </p>
          <button
            type="button"
            onClick={onBack}
        ...
        <div className="grid h-full min-h-0 grid-cols-1 gap-3 overflow-hidden xl:grid-cols-[minmax(0,1fr)_310px] 2xl:grid-cols-[minmax(0,1fr)_340px]">
          <QuizStage current={current} activeIndex={activeIndex} total={total} answered={answers[activeIndex] !== undefined}>
            <div className="flex h-full min-h-0 overflow-hidden">
              {current?.type === "model" ? (
                <ModelQuestionViewer modelSrc={current.modelSrc} pinLabel={current.pinLabel} />
              ) : (
                <PartsReviewVisual />
              )}
            </div>
    ...
    <div className="flex shrink-0 flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 max-w-4xl">
        ...
        <div className="mt-1 text-[13px] text-white/45">
          {current?.type === "model" ? "Inspect the pinned 3D model, then select the correct answer." : "Select one answer. It will auto-advance."}
        </div>
      </div>
...
function NavigatorCard({ questions, answers, activeIndex, setActiveIndex, total }) {
  return (
    <GlassPanel className="p-3">
      ...
      <div className="mt-1 text-[11px] text-white/50">Green dots mark 3D pin questions.</div>
