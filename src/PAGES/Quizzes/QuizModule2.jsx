...
/* ───────────────────────────────────────────────────────────── */
/* PREPARED MODULE 2 QUESTIONS */
/* ───────────────────────────────────────────────────────────── */

const MODULE2_QUESTIONS = [
  {
    type: "choice",
    q: "What is the correct first step in the Module 2 PC assembly sequence?",
    options: [
      "Install the CPU to the motherboard",
      "Install the PSU to the case",
      "Install the HDD to the case",
      "Install the motherboard to the case",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "After installing the CPU, which component is installed next in the Module 2 sequence?",
    options: ["RAM to motherboard", "PSU to case", "HDD to case", "Full assembly"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which is the correct Module 2 assembly sequence?",
    options: [
      "CPU → RAM → SSD → Motherboard → HDD → PSU → Full Assembly",
      "PSU → HDD → CPU → RAM → SSD → Motherboard → Full Assembly",
      "RAM → CPU → PSU → HDD → SSD → Motherboard → Full Assembly",
      "Motherboard → PSU → CPU → RAM → SSD → HDD → Full Assembly",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Where should the CPU be installed?",
    options: ["On the motherboard CPU socket", "Inside the PSU", "On the HDD tray", "On the case side panel"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What should be done after placing the CPU correctly?",
    options: ["Secure or lock it in place", "Remove the motherboard", "Disconnect the PSU", "Place it on the case floor"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Where should RAM be installed?",
    options: ["In the motherboard DIMM/RAM slot", "In the PSU slot", "In the HDD bay", "On the case fan mount"],
    answerIndex: 0,
  },
  {
    type: "choice",
    ...
    options: [
      ...
      "Protects the components physically",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "In Module 2, after RAM is placed, what component is assembled next?",
    options: ["SSD to motherboard", "HDD to case", "PSU to case", "Full assembly"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What is the purpose of an SSD?",
    options: [
      "Stores data and loads files faster than many traditional hard drives",
      "Converts electricity for the PC",
  ...
  {
    type: "choice",
    q: "After CPU, RAM, and SSD are installed, what happens next in Module 2?",
    options: [
      "Motherboard is installed into the case",
      "CPU is removed from the motherboard",
      "PSU is removed from the case",
      "RAM is placed on the floor",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Why should the motherboard be aligned carefully inside the case?",
    options: ["To match mounting points and prevent damage", "To increase the RAM capacity", "To make the CPU faster", "To erase stored files"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Where is the HDD installed in the assembly sequence?",
    options: ["Into the case drive bay or storage area", "Directly into the CPU socket", "Inside the RAM slot", "On top of the keyboard"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What does the PSU provide to the computer?",
    options: ["Electrical power", "Temporary memory", "Long-term file storage only", "Display output only"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "In Module 2, which step comes right before Full Assembly?",
    options: ["PSU to Case", "CPU to Motherboard", "RAM to Motherboard", "SSD to Motherboard"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "What does the Full Assembly scene represent?",
    options: [
      "All required parts are placed in their correct final positions",
      "Only the CPU is installed",
      "The PC is being disassembled",
      "Only the RAM is removed",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "If a user skips CPU installation and tries to continue the assembly, what is wrong?",
    options: [
      "The assembly sequence is incomplete",
      "The PSU becomes faster",
      "The HDD automatically installs itself",
      "The motherboard no longer needs power",
    ],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which part acts as the main board where CPU, RAM, and other components connect?",
    options: ["Motherboard", "HDD", "PSU", "Case"],
    answerIndex: 0,
  },
  {
    type: "choice",
    q: "Which component physically protects and holds internal PC components?",
...
const MODEL_QUESTIONS = [
  ...
  },
  {
    ...
    options: ["PSU", "Motherboard", "SSD", "RAM"],
    answerIndex: 2,
  },
  {
    type: "model",
    ...
    options: ["Motherboard", "HDD", "PSU", "CPU"],
    answerIndex: 0,
  },
  {
    type: "model",
    ...
    options: ["RAM", "HDD", "CPU", "Motherboard"],
    answerIndex: 1,
  },
  {
    type: "model",
    ...
    options: ["Power Supply Unit", "SSD", "RAM", "CPU"],
    answerIndex: 0,
  },
  {
    type: "model",
...
function buildQuizQuestions() {
  ...
  MODULE2_QUESTIONS.forEach((question, index) => {
    mixed.push(question);
...
function AssemblySequenceVisual() {
  const steps = ["CPU", "RAM", "SSD", "MB", "HDD", "PSU", "Full"];

  return (
    <div className="flex min-h-0 flex-1 flex-col justify-center overflow-hidden rounded-[24px] border border-white/10 bg-[#092532]/70 p-8 shadow-[inset_0_0_80px_rgba(0,255,180,0.04)]">
      <div className="text-[12px] font-bold uppercase tracking-[0.24em] text-[#b7fff0]/70">Assembly Flow</div>
      <div className="mt-5 grid grid-cols-7 gap-2">
        {steps.map((step, index) => (
          <div key={step} className="relative">
            <div className="flex h-20 items-center justify-center rounded-2xl border border-[#00ffb4]/20 bg-[#00ffb4]/8 text-base font-black text-white">
              {step}
            </div>
...
export default function QuizModule2({ onBack, onQuizComplete }) {
      ...
      <FullscreenShell>
        <CenteredPanel>
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-white/60">
            {accessError ? accessError : "Finish Module 2 first before taking this quiz."}
          </p>
          <button
            type="button"
            onClick={onBack}
        ...
        <div className="grid h-full min-h-0 grid-cols-1 gap-3 overflow-hidden xl:grid-cols-[minmax(0,1fr)_310px] 2xl:grid-cols-[minmax(0,1fr)_340px]">
          <QuizStage current={current} activeIndex={activeIndex} total={total} answered={answers[activeIndex] !== undefined}>
            <div className="flex h-full min-h-0 overflow-hidden">
              {current?.type === "model" ? <ModelQuestionViewer modelSrc={current.modelSrc} /> : <AssemblySequenceVisual />}
            </div>
      ...
      <div className="grid min-h-[74px] items-center gap-3 px-4 py-2.5 lg:grid-cols-[auto_minmax(0,1fr)_auto]">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill label="Assembly Quiz" />
          ...
          <div className="mt-1.5 flex min-w-0 items-end gap-3">
            <div className="min-w-0">
              <h1 className="truncate text-[24px] font-black leading-none tracking-tight text-white lg:text-[30px]">{QUIZ_SETTINGS.title}</h1>
              <div className="mt-1 truncate text-sm text-white/50">{QUIZ_SETTINGS.desc}</div>
            </div>
          </div>
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 max-w-4xl">
        ...
        <h2 className="mt-1 text-[22px] font-black leading-tight tracking-tight text-white lg:text-[28px]">{current?.q}</h2>
        ...
        {current.options.map((option, index) => {
          ...
          return (
            <motion.button
              ...
              className={[
                "group relative min-h-[62px] w-full overflow-hidden rounded-[20px] border p-3.5 text-left transition focus:outline-none focus:ring-2 focus:ring-[#00ffb4]/30",
        ...
        {Array.from({ length: total }).map((_, index) => {
          ...
          return (
            <button
              ...
              className={[
                "relative h-9 rounded-xl border text-xs font-black transition focus:outline-none focus:ring-2 focus:ring-[#00ffb4]/30",
...
function TimerCard({ time, status }) {
  return (
    <div className="rounded-[18px] border border-white/10 bg-white/7 px-5 py-2.5 text-center shadow-[0_18px_55px_rgba(0,0,0,0.25)] backdrop-blur-xl">
