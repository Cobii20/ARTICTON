        ...
        <div className="flex shrink-0 items-center justify-between border-b border-[#1a2438] px-4 py-4">
          {open ? (
            <div>
              ...
              <div className="text-[11px] text-[#7a8ba8]">Any order • INTEL Platform</div>
            </div>
          ) : null}
          <button
            type="button"
...
export default function INTELFullDisassemblyPracticalTest({ onFinish, onBack, onLogout }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        console.error("Error fetching INTEL Disassembly Practical Test profile:", error);
      }
    });
    return () => unsub();
  }, []);
