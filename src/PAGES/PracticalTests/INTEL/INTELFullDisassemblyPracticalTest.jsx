...
/*     are enforced from the PDF-adapted order: PSU first, then HDD,  */
/*     RAM, GPU, and motherboard. Once it is on the table, the        */
/*     app-modeled CPU/SSD removals finish the sequence.              */
...
/*     Sequence errors and elapsed time after the grace period reduce the grade. */
/*   - A results screen replaces the certificate, showing a grade,    */
/*     total time, and a breakdown of mistakes.                       */
/* ================================================================== */

const REMOVAL_SEQUENCE = DISASSEMBLY_SEQUENCE;
...
/* Physical dependency map for the practical test. The PDF desktop
   sequence removes the PSU before drives, RAM, expansion cards, and the
   motherboard. ARTICTON keeps app-modeled SSD/CPU removals after the
   motherboard reaches the worktable. */
const PREREQUISITES = DISASSEMBLY_PREREQUISITES;
...
function PartModel({
  ...
  const releaseComponent = useCallback(() => {
    ...
    if (pointerTravel < MAGNETIC_FIELD_MIN_POINTER_TRAVEL_PX) {
      ...
      onInteractionMessage(
        `${COMPONENT_LABELS[part.key]} was released without a meaningful drag. No score was deducted.`
      );
      publishTelemetry();
      return;
    }

    onFumble?.(part.key, { feedbackOnly: true });
...
export default function INTELFullDisassemblyPracticalTest({ onFinish, onBack }) {
  ...
  const finishTest = useCallback(
    async (
      ...
      let finalResult;
      try {
        finalResult = await finishAuthoritativePractical({ attemptId: practicalAttemptIdRef.current, practicalId: "intelDisassembly", wrongOrderCount: finalWrongOrder, completedParts: finalCompletedParts });
      } catch (error) {
        console.error("Error finalizing INTEL Disassembly Practical Test:", error);
      ...
      if (finalResult.passed) {
        setAchievementToast(resolveAchievement("intelDisassembly"));
  ...
  const handleInvalidClick = useCallback(
    (partKey) => {
      ...
      setValidationMessage(
        `${COMPONENT_LABELS[partKey]} is not accessible yet. A prerequisite component is missing.`
      );
    },
    [settings.sound]
  );
  ...
  const handleStartTest = useCallback(async () => {
    ...
    try {
      practicalAttemptIdRef.current = await startAuthoritativePractical("intelDisassembly");
      setShowIntro(false);
      setTestActive(true);
      ...
      console.error("Error starting INTEL Disassembly Practical Test:", error);
          ...
          <div className="relative flex h-full w-full flex-col overflow-hidden">
            ...
            <div className="relative z-[120] mt-3 px-6 md:px-10">
              <div className="flex w-full flex-wrap items-center justify-between gap-4 rounded-[22px] border border-[#1a2438] bg-[#0b1220]/86 px-6 py-4 shadow-[0_18px_60px_rgba(0,0,0,0.30)] backdrop-blur-xl">
                ...
                <div className="flex flex-wrap items-center justify-end gap-3">
                  {validationMessage ? (
                    <div className="articton-validation max-w-[520px] rounded-2xl border border-[#FFD41C]/20 bg-[#FFD41C]/8 px-4 py-2 text-xs font-semibold text-[#dffef5]">
                      {validationMessage}
                    </div>
                  ) : null}
