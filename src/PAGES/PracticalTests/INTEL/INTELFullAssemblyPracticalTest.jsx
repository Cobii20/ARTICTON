        ...
        <div className="relative">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-[#1a2438] bg-white/[0.03] p-4">
              ...
              <div className="mt-2 text-xs leading-5 text-[#9fb0ca]">Sequence errors cost points; placement misses only block progress until corrected.</div>
            </div>
          </div>
...
export default function INTELFullAssemblyPracticalTest({ onFinish, onBack }) {
  ...
  const finishTest = useCallback(
    async (
      ...
      let finalResult;
      try {
        finalResult = await finishAuthoritativePractical({ attemptId: practicalAttemptIdRef.current, practicalId: "intelAssembly", wrongOrderCount: finalWrongOrder, completedParts: finalCompletedParts });
      } catch (error) {
        console.error("Error finalizing INTEL Assembly Practical Test:", error);
      ...
      if (finalResult.passed) {
        setAchievementToast(resolveAchievement("intelAssembly"));
  ...
  const handleStartTest = useCallback(async () => {
    ...
    try {
      practicalAttemptIdRef.current = await startAuthoritativePractical("intelAssembly");
      setShowIntro(false);
      setTestActive(true);
      ...
      console.error("Error starting INTEL Assembly Practical Test:", error);
