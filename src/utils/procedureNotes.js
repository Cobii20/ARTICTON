...
export const PROCEDURE_DETAILS = {
  disassembly: [
    {
...
export function getProcedureText(mode) {
...
export function getComponentProcedureNote(mode, componentOrStep) {
  const details = PROCEDURE_DETAILS[mode] || [];
  const text = normalizeTerm(componentOrStep);

  if (!details.length) return null;

  const findByTitle = (terms) =>
    details.find((item) =>
      terms.some((term) => normalizeTerm(item.title).includes(term))
    );

  if (mode === "assembly") {
    if (text.includes("cpu")) return findByTitle(["cpu"]);
    if (text.includes("ram") || text.includes("ssd")) return findByTitle(["ram", "ssd"]);
    if (text.includes("motherboard")) return findByTitle(["mount motherboard"]);
    if (text.includes("psu") || text.includes("hdd")) return findByTitle(["psu", "drives"]);
    if (text.includes("gpu")) return findByTitle(["gpu"]);
  }

  if (mode === "disassembly") {
    if (text.includes("gpu")) return findByTitle(["gpu"]);
    if (text.includes("ssd") || text.includes("hdd") || text.includes("ram")) {
      return findByTitle(["storage", "memory"]);
    }
    if (text.includes("cpu")) return findByTitle(["cpu"]);
    if (text.includes("psu") || text.includes("motherboard")) {
      return findByTitle(["psu", "motherboard"]);
    }
  }
